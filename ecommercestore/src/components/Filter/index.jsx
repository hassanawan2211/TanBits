...
const Filter = ({ onSortChange, onSearch, onShowChange, totalItemsCount, showPerPage }) => {
  ...
  return (
    <div className="py-8 w-full flex justify-between items-center bg-[#F9F1E7]">
      <div className="flex items-center max-lg:justify-between">
        <img src={filtericon} alt="" className="ml-9"/>
        <h3 className="px-3 max-md:px-1">Filter</h3>
        ...
        <p className="px-3 max-md:hidden max-lg:hidden">
          Showing 1–{ showPerPage} of {totalItemsCount} results
        </p>
      </div>
      <div className="flex items-center">
        <p className="px- max-md:hidden max-lg:hidden">Show</p>
        <input
          type="number"
          value={showPerPage}
          onChange={handleShowChange}
          className="w-[55px] h-[30px] px-2 text-black text-center max-md:hidden max-lg:hidden"
        />
        <p className="px-2 max-md:hidden">Sort by</p>
        <select name="sortBy" value={`${sortBy}-${sortOrder}`} onChange={handleSortChange} className="w-[188px] h-[30px] text-center border-none rounded-md">
      ...
      <div className="flex items-center">
        <input
          ...
          className="w-[188px] h-[30px] mr-9 text-left p-2 text-black max-md:hidden"
        />
      </div>
    </div>
  );
